...
void loop() {
  ...
  servo.write(90);
  // run continuous scan at intervals
...
void avoidObstacle() {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    if ((distance >= maxDist) && (distance<1200)){
      maxDist = distance;
      reqAngle = ((i * 45)-90);
      reqI = i;
    } 
...
void turnCar(int angle) {
  ...
  int PWM_VALUE = ROTATE_PWM;
  analogWrite(ENA, PWM_VALUE);
  analogWrite(ENB, PWM_VALUE);
  Serial.print("Calculated Time for turn = ");
  Serial.println(750 * 255 / PWM_VALUE * abs(angle) / 90);
  delay(750 * 255 / PWM_VALUE * abs(angle) / 90);
